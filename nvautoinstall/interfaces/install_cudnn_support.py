...
class InstallCuDNNSupport:
    """
    Interface to install the CuDNN library.
    """
                ...
                if Objc_HandleRPMFusionRepositories.conn():
                    ...
                    if data is False:
                        ...
                        if Objc_HandleCudaInstallation.avbl():
                            ...
                            section("CHECKING AVAILABILITY OF CuDNN REPO...")
                            if Objc_HandleCuDNNInstallation.repo_installed():
                                ...
                                if Objc_HandleCuDNNInstallation.add_repo():
                                    ...
                                    failure("CuDNN repo could not be added")
                                    return

                            section("INSTALLING CuDNN LIBRARY...")
                            detected_versions, package_name = Objc_HandleCuDNNInstallation.search(version)
                            if package_name is not None:
                                success("Found CuDNN package " + package_name)
                                section("INSTALLING CuDNN PACKAGE...")
                                if Objc_HandleCuDNNInstallation.main(package_name):
                                    success("CuDNN library were successfully installed")
                                else:
                                    failure("CuDNN library could not be installed")
                                    general(
                                        "Please try executing `dnf update` with elevated privileges before this"  # noqa
                                    )
                            else:
                                failure("CuDNN library matching version " + version + " was not found. Detected versions:\n" + "\n".join("      * " + v for v in detected_versions))
                                general(
